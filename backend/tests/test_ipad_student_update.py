...
class TestIPadUpdate(TestAuth):
    ...
    def test_update_ipad_all_fields(self, auth_headers, test_ipad):
        ...
        response = requests.put(f"{BASE_URL}/api/ipads/{test_ipad['id']}", json=update_data, headers=auth_headers)
        ...
        print("✓ iPad update all fields: PASS")

    def test_update_ipad_partial_fields(self, auth_headers, test_ipad):
        """Test updating only some iPad fields"""
        update_data = {"karton": "Karton-Partial-Update", "pencil": "Pencil-Partial-Update"}
        ...
        print("✓ iPad partial update: PASS")
    ...
    def test_update_ipad_duplicate_itnr(self, auth_headers, test_ipad):
        ...
        try:
            ...
            response = requests.put(
                f"{BASE_URL}/api/ipads/{test_ipad['id']}", json={"itnr": other_ipad["itnr"]}, headers=auth_headers
            )
            assert response.status_code == 400, f"Expected 400 for duplicate ITNr, got {response.status_code}"
            print("✓ iPad duplicate ITNr rejected: PASS")
        finally:
            # Cleanup
...
class TestStudentUpdate(TestAuth):
    ...
    def test_update_student_personal_data(self, auth_headers, test_student):
        ...
        response = requests.put(f"{BASE_URL}/api/students/{test_student['id']}", json=update_data, headers=auth_headers)
        ...
        print("✓ Student personal data update: PASS")
    ...
    def test_update_student_guardian1(self, auth_headers, test_student):
        ...
        print("✓ Student guardian 1 update: PASS")
    ...
    def test_update_student_guardian2(self, auth_headers, test_student):
        ...
        print("✓ Student guardian 2 update: PASS")

    def test_update_student_not_found(self, auth_headers):
        """Test updating non-existent student"""
        response = requests.put(
            f"{BASE_URL}/api/students/non-existent-id", json={"sus_kl": "12C"}, headers=auth_headers
        )
        assert response.status_code == 404, f"Expected 404, got {response.status_code}"
        print("✓ Student not found returns 404: PASS")

    def test_update_student_empty_request(self, auth_headers, test_student):
        """Test updating student with empty request body"""
        response = requests.put(f"{BASE_URL}/api/students/{test_student['id']}", json={}, headers=auth_headers)
        assert response.status_code == 400, f"Expected 400 for empty update, got {response.status_code}"
        print("✓ Student empty update rejected: PASS")
...
class TestNameChangePropagation(TestAuth):
    ...
    def test_student_name_change_updates_assignments(self, auth_headers):
        ...
        # Create student
        student_data = {"sus_vorn": f"OriginalVorname-{unique_id}", "sus_nachn": f"OriginalNachname-{unique_id}"}
        student_response = requests.post(f"{BASE_URL}/api/students", json=student_data, headers=auth_headers)
        assert student_response.status_code == 200
        student = student_response.json()

        # Create iPad
        ipad_data = {"itnr": f"PROP-IPAD-{unique_id}", "snr": f"PROP-SNR-{unique_id}"}
        ...
        # Update student name
        new_name_data = {"sus_vorn": f"NewVorname-{unique_id}", "sus_nachn": f"NewNachname-{unique_id}"}
        update_response = requests.put(
            f"{BASE_URL}/api/students/{student['id']}", json=new_name_data, headers=auth_headers
        )
        ...
        assert (
            student_assignment["student_name"] == expected_name
        ), f"Expected '{expected_name}', got '{student_assignment['student_name']}'"

        print("✓ Student name change propagates to assignments: PASS")
...
class TestIPadITNrChangePropagation(TestAuth):
    ...
    def test_ipad_itnr_change_updates_assignments(self, auth_headers):
        ...
        # Create student
        student_data = {"sus_vorn": f"ITNrTestVorname-{unique_id}", "sus_nachn": f"ITNrTestNachname-{unique_id}"}
        student_response = requests.post(f"{BASE_URL}/api/students", json=student_data, headers=auth_headers)
        assert student_response.status_code == 200
        student = student_response.json()
        ...
        ipad_data = {"itnr": original_itnr, "snr": f"ITNR-SNR-{unique_id}"}
        ipad_response = requests.post(f"{BASE_URL}/api/ipads", json=ipad_data, headers=auth_headers)
        assert ipad_response.status_code == 200
        ipad = ipad_response.json()
        ...
        update_response = requests.put(
            f"{BASE_URL}/api/ipads/{ipad['id']}", json={"itnr": new_itnr}, headers=auth_headers
        )
        assert update_response.status_code == 200
        ...
        print("✓ iPad ITNr change propagates to assignments: PASS")
