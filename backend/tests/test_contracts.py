...
class TestContractsAPI:
    ...
    def test_batch_delete_empty_list(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/contracts/batch-delete", headers=self.headers, json={"contract_ids": []}
        )
    ...
    def test_batch_delete_max_limit(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/contracts/batch-delete", headers=self.headers, json={"contract_ids": fake_ids}
        )
    ...
    def test_unassign_nonexistent_contract(self):
        ...
        response = requests.post(f"{BASE_URL}/api/contracts/nonexistent-id/unassign", headers=self.headers)
    ...
    def test_delete_nonexistent_contract(self):
        ...
        response = requests.delete(f"{BASE_URL}/api/contracts/nonexistent-id", headers=self.headers)
        assert response.status_code == 404, f"Expected 404, got {response.status_code}"
    ...
    def test_full_contract_workflow(self):
        ...
        student_response = requests.post(
            ...
            headers=self.headers,
            json={"sus_vorn": "TEST_ContractWorkflow", "sus_nachn": "Student", "sus_kl": "TEST-Class"},
        )
        ...
        files = {"file": (f"TEST_Contract_{test_itnr}.pdf", io.BytesIO(pdf_content), "application/pdf")}
        upload_headers = {"Authorization": f"Bearer {self.token}"}

        upload_response = requests.post(
            f"{BASE_URL}/api/assignments/{assignment_id}/upload-contract", headers=upload_headers, files=files
        )
        ...
        delete_response = requests.delete(f"{BASE_URL}/api/contracts/{contract_id}", headers=self.headers)
        ...
        assert (
            test_assignment_after.get("contract_id") is None
        ), f"BUG: Assignment contract_id should be null after contract deletion, got: {test_assignment_after.get('contract_id')}"

        print("Step 7: PASS - Assignment contract_id is null after contract deletion")

        # Cleanup: Dissolve assignment and delete iPad
        # First dissolve assignment
        dissolve_response = requests.post(f"{BASE_URL}/api/assignments/{assignment_id}/dissolve", headers=self.headers)
        print(f"Cleanup: Dissolved assignment: {dissolve_response.status_code}")

        # Delete iPad
        delete_ipad_response = requests.delete(f"{BASE_URL}/api/ipads/{ipad_id}", headers=self.headers)
...
class TestContractsBatchDeleteWithRealData:
    ...
    def test_batch_delete_with_assigned_contracts(self):
        ...
        student_response = requests.post(
            f"{BASE_URL}/api/students",
            headers=self.headers,
            json={"sus_vorn": f"TEST_Batch_{uuid.uuid4().hex[:4]}", "sus_nachn": "Student", "sus_kl": "TEST-Batch"},
        )
        if student_response.status_code != 200:
        ...
        try:
            ...
            for i in range(2):
                ...
                # Upload contract
                pdf_content = b"%PDF-1.4\n1 0 obj\n<<\n/Type /Catalog\n/Pages 2 0 R\n>>\nendobj\n2 0 obj\n<<\n/Type /Pages\n/Kids []\n/Count 0\n>>\nendobj\nxref\n0 3\n0000000000 65535 f\n0000000009 00000 n\n0000000058 00000 n\ntrailer\n<<\n/Size 3\n/Root 1 0 R\n>>\nstartxref\n115\n%%EOF"
                files = {"file": (f"TEST_BatchContract_{i}.pdf", io.BytesIO(pdf_content), "application/pdf")}
                upload_headers = {"Authorization": f"Bearer {self.token}"}
                upload_response = requests.post(
                    f"{BASE_URL}/api/assignments/{assignment_id}/upload-contract", headers=upload_headers, files=files
                )
            ...
            for assignment_id in created_assignments:
                ...
                if assignment:
                    assert (
                        assignment.get("contract_id") is not None
                    ), f"Assignment {assignment_id} should have contract_id"
            print("Verified all assignments have contract_ids")

            # Step 2: Batch delete contracts
            batch_response = requests.post(
                f"{BASE_URL}/api/contracts/batch-delete", headers=self.headers, json={"contract_ids": created_contracts}
            )
            ...
            for assignment_id in created_assignments:
                ...
                if assignment:
                    assert (
                        assignment.get("contract_id") is None
...
class TestContractsUnassign:
    ...
    def test_unassign_contract_not_assigned(self):
        ...
        if unassigned_contract:
            response = requests.post(
                f"{BASE_URL}/api/contracts/{unassigned_contract['id']}/unassign", headers=self.headers
            )
            ...
            print("PASS: Unassign on unassigned contract returns 400")
        else:
    ...
    def test_unassign_full_workflow(self):
        ...
        student_response = requests.post(
            f"{BASE_URL}/api/students",
            headers=self.headers,
            json={"sus_vorn": f"TEST_Unassign_{uuid.uuid4().hex[:4]}", "sus_nachn": "Student", "sus_kl": "TEST-UA"},
        )
        if student_response.status_code != 200:
            pytest.skip(f"Could not create student: {student_response.text}")
        student_id = student_response.json()["id"]
        ...
        try:
            ...
            unassign_response = requests.post(f"{BASE_URL}/api/contracts/{contract_id}/unassign", headers=self.headers)
            ...
            assert (
                test_assignment.get("contract_id") is None
