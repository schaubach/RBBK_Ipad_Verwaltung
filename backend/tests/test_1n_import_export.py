...
class TestExistingTestData:
    ...
    def test_students_with_assignment_counts(self, api_client):
        ...
        if "Peter Müller" in student_map:
            assert (
                student_map["Peter Müller"]["assignment_count"] == 3
        ...
        if "Max Mustermann" in student_map:
            assert (
                student_map["Max Mustermann"]["assignment_count"] == 2
...
class TestStudentAvailability:
    ...
    def test_available_students_excludes_at_limit(self, api_client):
        ...
        for student in available:
            assert (
                student["current_ipads"] < MAX_IPADS_PER_STUDENT
...
class TestIPadLimit:
    ...
    def test_cannot_assign_beyond_limit(self, api_client):
        ...
        response = api_client.post(
            ...
            json={"student_id": student_at_limit["id"], "ipad_id": available_ipads[0]["id"]},
        )

        # Should fail with error about limit
        assert response.status_code == 400, f"Expected 400 error for limit, got {response.status_code}: {response.text}"

        assert (
            "limit" in response.text.lower()
            or "maximum" in response.text.lower()
            or str(MAX_IPADS_PER_STUDENT) in response.text
...
class TestExportInventory:
    ...
    def test_export_returns_xlsx(self, api_client):
        ...
        assert (
            "spreadsheet" in content_type or "excel" in content_type or "octet-stream" in content_type
...
class TestImportInventory:
    ...
    def test_re_import_exported_data(self, api_client):
        ...
        assert (
            result.get("ipads_skipped", 0) > 0 or result.get("students_skipped", 0) > 0
        ), "Re-import should skip existing data"

        print("✓ Re-import handled existing data correctly")
...
class TestImport1toN:
    ...
    def test_import_merges_same_student(self, api_client):
        ...
        assert result.get("ipads_created", 0) == 2, f"Expected 2 iPads created, got {result.get('ipads_created', 0)}"
        assert (
            result.get("students_created", 0) == 1
        ), f"Expected 1 student created (merged), got {result.get('students_created', 0)}"
        assert (
            result.get("assignments_created", 0) == 2
        ), f"Expected 2 assignments created, got {result.get('assignments_created', 0)}"

        print("✓ 1:n import correctly merged same student")
        ...
        assert (
            test_student.get("assignment_count", 0) == 2
        ), f"Test student should have 2 assignments, got {test_student.get('assignment_count', 0)}"

        print("✓ Merged student correctly has 2 iPad assignments")
...
class TestLimitEnforcementOnImport:
    ...
    def test_import_respects_limit(self, api_client):
        ...
        result = response.json()
        print("Limit test import result:")
        print(f"  - iPads created: {result.get('ipads_created', 0)}")
        print(f"  - Assignments created: {result.get('assignments_created', 0)}")
        ...
        assert result.get("ipads_created", 0) == 5, f"Expected 5 iPads created, got {result.get('ipads_created', 0)}"
        assert (
            result.get("assignments_created", 0) == MAX_IPADS_PER_STUDENT
        ), f"Expected {MAX_IPADS_PER_STUDENT} assignments (limit), got {result.get('assignments_created', 0)}"
        assert (
            result.get("assignments_skipped_limit", 0) == 2
