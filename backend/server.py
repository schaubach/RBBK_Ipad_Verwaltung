...
@api_router.post("/assignments/upload-combined", response_model=UploadResponse)
async def upload_combined_assignments(file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    """
    Upload Excel with combined Student+iPad data for 1:n assignments.
    If a student appears multiple times with different iPads, all iPads will be assigned.
    Expected columns: sus_vorn, sus_nachn, sus_kl, itnr, snr, ...
    """
    try:
        contents = await file.read()
        df = pd.read_excel(io.BytesIO(contents))
        ...
        # Group by student to handle 1:n relationships
        student_ipad_map = {}  # {(vorn, nachn): [ipad_data, ...]}
        
        for idx, row in df.iterrows():
            sus_vorn = str(row.get('sus_vorn', '')).strip()
            sus_nachn = str(row.get('sus_nachn', '')).strip()
            itnr = str(row.get('itnr', '')).strip()
            snr = str(row.get('snr', '')).strip()
            
            # Skip if student or iPad data is missing
            if not sus_vorn or sus_vorn == 'nan' or not sus_nachn or sus_nachn == 'nan':
                skipped_count += 1
                details.append(f"Row {idx+2}: Missing student name - skipped")
                continue
                
            if not itnr or itnr == 'nan' or not snr or snr == 'nan':
                skipped_count += 1
                details.append(f"Row {idx+2}: Missing iPad data for {sus_vorn} {sus_nachn} - skipped")
                continue
            
            # Build student key
            student_key = (sus_vorn, sus_nachn)
            
            if student_key not in student_ipad_map:
                student_ipad_map[student_key] = {
                    'student_data': row,
                    'ipads': []
                }
            
            # Add iPad to this student
            student_ipad_map[student_key]['ipads'].append({
                'itnr': itnr,
                'snr': snr,
                'row_data': row
            })
        
        # Process each student with their iPads
        for (sus_vorn, sus_nachn), data in student_ipad_map.items():
            row = data['student_data']
            ipads = data['ipads']
            ...
            if not existing_student:
                ...
                student = Student(
                    ...
                    sus_vorn=sus_vorn,
                    sus_nachn=sus_nachn,
                    sus_kl=str(row.get('sus_kl', '')),
                    sus_geb=str(row.get('sus_geb', '')),
                    sus_str_hnr=str(row.get('sus_str_hnr', '')),
                    sus_plz=str(row.get('sus_plz', '')),
                    sus_ort=str(row.get('sus_ort', ''))
                )
                student_dict = prepare_for_mongo(student.dict())
                await db.students.insert_one(student_dict)
                student_id = student.id
                details.append(f"Student {sus_vorn} {sus_nachn} created")
            else:
                student_id = existing_student["id"]
                details.append(f"Student {sus_vorn} {sus_nachn} already exists")
            
            # Check current assignment count
            current_assignments = await db.assignments.count_documents({
                "student_id": student_id,
                "is_active": True
            })
            
            # Assign iPads (up to MAX_IPADS_PER_STUDENT)
            for ipad_data in ipads:
                if current_assignments >= MAX_IPADS_PER_STUDENT:
                    skipped_count += 1
                    details.append(f"iPad {ipad_data['itnr']} for {sus_vorn} {sus_nachn}: Limit reached ({MAX_IPADS_PER_STUDENT} iPads)")
                    continue
                
                # Check if iPad exists, create if not
                existing_ipad = await db.ipads.find_one({
                    "itnr": ipad_data['itnr'],
                    "user_id": current_user["id"]
                })
                
                if not existing_ipad:
                    # Create iPad
                    ipad = iPad(
                        user_id=current_user["id"],
                        itnr=ipad_data['itnr'],
                        snr=ipad_data['snr'],
                        typ=str(ipad_data['row_data'].get('typ', '')),
                        status='ok'
                    )
                    ipad_dict = prepare_for_mongo(ipad.dict())
                    await db.ipads.insert_one(ipad_dict)
                    ipad_id = ipad.id
                    details.append(f"iPad {ipad_data['itnr']} created")
                else:
                    ipad_id = existing_ipad["id"]
                    # Check if iPad is already assigned
                    if existing_ipad.get("current_assignment_id"):
                        skipped_count += 1
                        details.append(f"iPad {ipad_data['itnr']} already assigned - skipped")
                        continue
                ...
                await db.ipads.update_one(
                    {"id": ipad_id},
                    {"$set": {"current_assignment_id": assignment.id}}
                )
                ...
                details.append(f"✓ Assigned iPad {ipad_data['itnr']} to {sus_vorn} {sus_nachn}")
        
        return UploadResponse(
            message=f"Processed {processed_count} assignments, skipped {skipped_count}",
            processed_count=processed_count,
            skipped_count=skipped_count,
            details=details
        )
        ...
        raise HTTPException(status_code=500, detail=f"Error processing combined upload: {str(e)}")
...
async def generate_contracts(
    ...
    try:
        ...
        if success_count == 0:
            raise HTTPException(
                status_code=500, 
                detail=f"Keine Verträge erstellt. Fehler: {'; '.join(errors[:5])}"
            )
...
app.add_middleware(
    ...
    allow_origins=os.environ.get('CORS_ORIGINS', 'http://localhost:3000,https://student-ipad-mgmt.preview.emergentagent.com').split(','),
